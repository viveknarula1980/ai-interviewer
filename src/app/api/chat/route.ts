...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isFirstQuestion) {
      if (isResumeBased) {
        prompt = `You are a professional AI interviewer. The candidate has uploaded the following resume:
---
${resumeText}
---
Generate ONE professional, engaging opening interview question based on their actual experience and background. 
The question should be at ${difficulty} difficulty level.
Start with a brief, warm welcome (1 sentence) then ask the question.
Speak as if you are on a real phone interview. Be natural and conversational. Do NOT use bullet points or markdown.`;
      } else {
        prompt = `You are a professional AI interviewer. Generate ONE professional opening interview question for a ${difficulty} level ${subject} interview.
Start with a brief warm welcome (one sentence) then ask the question directly.
Speak naturally as if on a phone interview. Do NOT use bullet points or markdown. Keep it under 3 sentences total.`;
      ...
      prompt = `You are a professional AI interviewer conducting a ${difficulty} level ${subject} interview.${contextNote}\n\nConversation so far:\n${historyContext}\n\nCandidate's latest answer: "${transcript}"\n\n${questionContext}\n${questionNumber > totalQuestions ? `The interview is now complete. Give a professional, warm closing statement. Thank the candidate. Tell them their feedback report is now being generated. Keep it to 2-3 sentences. No bullet points.` : `Evaluate the candidate's answer. If it was strong and complete, acknowledge it briefly (1 sentence) then ask the next interview question. If it was shallow or missed the point, probe with a focused follow-up question. \nAlways stay in character as the interviewer. Keep your total response under 4 sentences. No bullet points or markdown.`}`;
    }
